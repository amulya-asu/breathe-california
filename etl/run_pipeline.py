...
def main():
    ...
    # Log missing stations (no fallback — only real AirNow data is used)
    if len(df_live) > 0:
        live_keys = set()
        for _, r in df_live.iterrows():
            live_keys.add((r["county_fips"], r["latitude"], r["longitude"]))
        n_missing = sum(1 for _, s in stations.iterrows()
                        if (s["county_fips"], s["latitude"], s["longitude"]) not in live_keys)
        if n_missing:
            print(f"  {n_missing} stations had no live AirNow data (will use last known reading)")
